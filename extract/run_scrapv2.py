...
Usage:
    uv run extract/run_scraper.py   --config extract/categories.yml   --output-dir scraped_data   --max-pages 200   --delay 2.0   --workers 4
"""
...
def _scrape_one(cat: dict, output_dir: str, customer_id: str, client_id: str,
                 max_pages: int, delay_seconds: float) -> CategoryResult:
    ...
    try:
        ...
        result.rows = rows
        result.success = True
        log.info("Done: %s -> %d rows", name, rows)
    except Exception as exc:
        # One bad category (typo'd slug, transient network failure that
        # exhausted retries, etc.) must not take down the other 13.
...
def run_all(
    ...
    # Categories are independent (separate params, separate cursor chains,
    # separate output files), so they scrape concurrently. Pages *within*
    # a category still go one at a time inside scraper.scrape() -- that
    # part can't be parallelized because of cursor-based pagination.
    with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="cat") as pool:
        future_to_cat = {
            pool.submit(
                _scrape_one, cat, output_dir, customer_id, client_id, max_pages, delay_seconds
            ): cat.get("name", cat["category_slug"])
            for cat in categories
        }

        for i, future in enumerate(as_completed(future_to_cat), start=1):
            name = future_to_cat[future]
            log.info("[%d/%d] finished: %s", i, len(categories), name)
...
def main() -> int:
    ...
    parser.add_argument("--workers", type=int, default=4,
                         help="number of categories to scrape concurrently")
