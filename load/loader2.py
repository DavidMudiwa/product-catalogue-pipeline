"""
Loads Takealot scraper CSV output into BigQuery or DuckDB.
...
Usage (DuckDB - default):
    uv run load/loader2.py --input ./scraped_data/tvs_20260715_143000.csv
    uv run load/loader2.py --input ./scraped_data/ --dry-run

Usage (BigQuery production):
    uv run load/loader2.py --input ./scraped_data/ --config load/config.yaml --target prod --dry-run
"""
...
# DuckDB type mapping from BigQuery types
DUCKDB_TYPE_MAP = {
    "INTEGER": "BIGINT",
    "STRING": "VARCHAR",
    "FLOAT": "DOUBLE",
...
DEFAULT_DUCKDB_PATH = "product_catalog/dev.duckdb"
DEFAULT_DUCKDB_SCHEMA = "products_raw"
DEFAULT_DUCKDB_TABLE = "products"
...
class Config:
    ...
    table: str | None = None
    # credentials_path is a legacy fallback only -- most GCP orgs created after
    # May 2024 have iam.disableServiceAccountKeyCreation enforced by default,
    # so a JSON key usually isn't even obtainable. Leave this unset and use
    # Application Default Credentials instead (see impersonate_service_account
    # below for the no-key production pattern).
    ...
    @classmethod
    def load(cls, config_path: str | None, cli_overrides: dict[str, Any], target: str) -> "Config":
        raw: dict[str, Any] = {}
        ...
        env_map = {
            ...
            "dataset": "BQ_DATASET",
            "table": "BQ_TABLE",
            # NOTE: deliberately NOT mapped to GOOGLE_APPLICATION_CREDENTIALS.
            # That's Google's own standard env var -- google.auth.default()
            # already reads it automatically for plain ADC, including the
            # authorized_user format written by `gcloud auth application-
            # default login`. If we also mapped it here, setting it (e.g. to
            # point ADC at a mounted credentials file in Docker) would get
            # reinterpreted as a legacy service-account-key path instead,
            # and crash with a MalformedError since the two JSON formats
            # aren't interchangeable. BQ_CREDENTIALS_PATH is a distinct,
            # explicit opt-in for the legacy JSON-key fallback only.
        ...
        if target == TARGET_PROD:
            ...
            if missing:
                raise ValueError(
                    f"Missing required BigQuery config for --target prod: {missing}. "
                    f"Set via --{missing[0].replace('_', '-')}, env var, or config file."
                )

        # For DuckDB, ensure defaults are set
        if target == TARGET_DUCKDB:
            for key, default in [
                ("duckdb_path", DEFAULT_DUCKDB_PATH),
                ("duckdb_schema", DEFAULT_DUCKDB_SCHEMA),
                ("duckdb_table", DEFAULT_DUCKDB_TABLE),
            ]:
                if not raw.get(key):
                    raw[key] = default
...
class Checkpoint:
    ...
    def __init__(self, checkpoint_dir: str, source_file: str, target: str = "duckdb"):
        os.makedirs(checkpoint_dir, exist_ok=True)
        safe_name = Path(source_file).name.replace(".", "_")
        # Target-specific checkpoint so DuckDB and BigQuery loads are independent
        self.path = Path(checkpoint_dir) / f"{safe_name}.{target}.checkpoint.json"
...
# --------------------------------------------------------------------------
# Loader Protocol -- common interface for BigQuery and DuckDB loaders
# --------------------------------------------------------------------------

class DataLoader(Protocol):
    """Protocol defining the interface that both BigQueryLoader and DuckDBLoader must implement."""
...
class DuckDBLoader:
    """DuckDB-backed loader. Mirrors the BigQuery loader interface so that
    process_file() can work with either target without changes.

    Uses a local DuckDB file with the schema/table structure matching BigQuery
    so the same data can be loaded to either target without transformation changes.
    """
    ...
    def conn(self):
        ...
        if self._conn is None:
            ...
            self._conn = duckdb.connect(self.config.duckdb_path)
        return self._conn
    ...
    @property
    def target_table_ref(self) -> str:
        return f"{self.config.duckdb_schema}.{self.config.duckdb_table}"

    @property
    def _full_qualified_target(self) -> str:
        """Fully qualified table reference for SQL."""
        return f'"{self.config.duckdb_schema}"."{self.config.duckdb_table}"'

    def _build_duckdb_schema(self) -> str:
        """Convert BigQuery RAW_SCHEMA to DuckDB CREATE TABLE DDL."""
        col_defs = []
        for field in RAW_SCHEMA:
            duckdb_type = DUCKDB_TYPE_MAP.get(field.field_type, "VARCHAR")
            nullable = "" if field.mode == "REQUIRED" else "NULL"
            col_defs.append(f'    "{field.name}" {duckdb_type} {nullable}'.strip())
        return ",\n".join(col_defs)

    def ensure_dataset(self) -> None:
        """Ensure the schema (namespace) exists in DuckDB."""
        self.conn.execute(f'CREATE SCHEMA IF NOT EXISTS "{self.config.duckdb_schema}"')
        log_with_fields(
            log, logging.INFO,
            "Ensured DuckDB schema exists",
            schema=self.config.duckdb_schema,
            database=self.config.duckdb_path,
        )

    def ensure_target_table(self) -> None:
        """Create the target table if it doesn't exist. DuckDB doesn't support
        native partitioning/clustering like BigQuery, but we create indexes on
        the merge key columns for fast upserts."""
        # Check if table exists
        result = self.conn.execute(f"""
            SELECT COUNT(*) FROM information_schema.tables
            WHERE table_schema = '{self.config.duckdb_schema}'
            AND table_name = '{self.config.duckdb_table}'
        """).fetchone()

        if result and result[0] > 0:
            log_with_fields(
                log, logging.INFO,
                "DuckDB target table already exists",
                table=self.target_table_ref,
            )
            return

        schema_ddl = self._build_duckdb_schema()
        create_sql = f"""
        CREATE TABLE {self._full_qualified_target} (
{schema_ddl}
        )
        """
        self.conn.execute(create_sql)

        # Create index on merge key columns for fast upserts
        idx_name = f"idx_{self.config.duckdb_table}_merge_key"
        self.conn.execute(f"""
            CREATE UNIQUE INDEX "{idx_name}" ON {self._full_qualified_target}
            (product_id, scraped_at)
        """)

        log_with_fields(
            log, logging.INFO,
            "Created DuckDB target table",
            table=self.target_table_ref,
            database=self.config.duckdb_path,
            merge_key_index=idx_name,
        )

    def create_staging_table(self, run_id: str) -> str:
        """Create a temporary staging table for this run."""
        staging_name = f"_staging_{run_id}"
        staging_ref = f'"{self.config.duckdb_schema}"."{staging_name}"'
        schema_ddl = self._build_duckdb_schema()
...
{schema_ddl}
    ...
    def load_chunk(self, chunk_path: str, staging_ref: str) -> str:
        """Load a CSV chunk into the DuckDB staging table."""
        # DuckDB can read CSV directly and insert
        self.conn.execute(f"""
            INSERT INTO {staging_ref}
            SELECT * FROM read_csv_auto('{chunk_path}')
        """)

        # Return a synthetic job ID for metrics consistency
        job_id = f"duckdb_{uuid.uuid4().hex[:12]}"
        return job_id

    def merge_staging_into_target(self, staging_ref: str, source_file: str) -> int:
        """Atomically upsert staging rows into the target table using DuckDB's
        INSERT OR REPLACE (or DELETE + INSERT pattern for MERGE semantics)."""
        target = self._full_qualified_target

        # DuckDB doesn't have a native MERGE statement in all versions,
        # so we use DELETE + INSERT for upsert semantics keyed on (product_id, scraped_at)
        key_cols = ", ".join(MERGE_KEY_COLUMNS)

        # First, delete existing rows in target that match the staging merge keys
        self.conn.execute(f"""
            DELETE FROM {target}
            WHERE (product_id, scraped_at) IN (
                SELECT product_id, scraped_at FROM {staging_ref}
            )
        """)

        # Then insert all rows from staging
        self.conn.execute(f"""
            INSERT INTO {target}
            SELECT * FROM {staging_ref}
        """)

        # Get row count from staging for logging
        count_result = self.conn.execute(f"SELECT COUNT(*) FROM {staging_ref}").fetchone()
        rows_affected = count_result[0] if count_result else 0

        log_with_fields(
            log, logging.INFO,
            "DuckDB MERGE (DELETE+INSERT) complete",
    ...
    def drop_staging_table(self, staging_ref: str) -> None:
        ...
        self.conn.execute(f"DROP TABLE IF EXISTS {staging_ref}")
...
def process_file(
    ...
    checkpoint = Checkpoint(config.checkpoint_dir, csv_path, target)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Load Takealot scraper CSVs into DuckDB (default) or BigQuery (production)."
    )
    ...
    parser.add_argument("--target", choices=[TARGET_DUCKDB, TARGET_PROD], default=TARGET_DUCKDB,
                        help=f"load target: '{TARGET_DUCKDB}' (default) for local DuckDB, '{TARGET_PROD}' for BigQuery production")
    ...
    parser.add_argument("--duckdb-path", default=None, help=f"path to DuckDB file (default: {DEFAULT_DUCKDB_PATH})")
    parser.add_argument("--duckdb-schema", default=None, help=f"DuckDB schema name (default: {DEFAULT_DUCKDB_SCHEMA})")
    parser.add_argument("--duckdb-table", default=None, help=f"DuckDB table name (default: {DEFAULT_DUCKDB_TABLE})")
    ...
    try:
        config = Config.load(args.config, cli_overrides, target=args.target)
    except ValueError as exc:
        log_with_fields(log, logging.ERROR, "Config error", error=str(exc))
        return 2

    # Log which target we're using
    if args.target == TARGET_PROD:
        log_with_fields(
            log, logging.INFO,
            "Loading to BigQuery production",
            target=args.target,
        ...
        log_with_fields(
            log, logging.INFO,
            f"Loading to DuckDB at {config.duckdb_path}",
            target=args.target,
            database=config.duckdb_path,
            schema=config.duckdb_schema,
    ...
    for csv_path in files:
        try:
            m = process_file(csv_path, loader, config, args.dry_run, args.target)
